...
function Basket({data,ismodal,setismodal,removeGrocery,cart}) {
  ...
  function cost(){
    let totalcost=0
    cart.forEach(item => totalcost+=item.cost)
    return totalcost
  }
 ...
 const showdata=()=>{
            ...
            
           {
            ...
            data.map(item=>(
              ...
              <div class="card-body">
                ...
                <h1>Price:- Rs.{item.cost}</h1>
                <button className="btn btn-outline-danger" onClick={()=>removeGrocery(item.id)}>Remove</button>
              </div>
            </div>
    ...
    return (
        ...
        <div className="row">
        <button style={{borderRadius:"40%"}} onClick={()=>setismodal(false)} className="btn btn-outline-danger mr-5 text-white text-dark ml-auto"><b><h1>X</h1></b></button>
        </div>
